...
import { EVENT_TYPES, METRICS, aggregate, drillDown, businessDate, periodLabel, eventSign } from './financial-engine.js'
...
export async function handleSupabaseRoute(request, { params }) {
  ...
  try {
    ...
    // Check optional authentication for operational reads/writes to enforce database-level isolation
    const auth = await authenticateRequest(request)
    const isProtected = !auth.error
    ...
    if (route === '/events/booking' && method === 'POST') {
      ...
      const event = {
        ...
        created_by: b.created_by || (auth.profile ? auth.profile.email : 'reception'),
        customer: b.customer,
    ...
    if (route === '/events/membership' && method === 'POST') {
      ...
      const date = b.business_date || businessDate()
      const bd = await ensureBusinessDay(db, centreId, date)
      if (bd.status === 'CLOSED') return cors(NextResponse.json({ error: 'Business day closed' }, { status: 400 }))
      ...
      const event = {
        ...
        centre_id: centreId, business_date: date, created_at: new Date().toISOString(), created_by: b.created_by || 'reception',
      ...
      await writeAudit(db, { action: 'SELL_MEMBERSHIP', actor: event.created_by, role: 'RECEPTION', centre_id: centreId, business_date: date, target_event_id: event.id, new_value: { code, value: membership.original_paise } })
    ...
    if (route === '/events/gift-card' && method === 'POST') {
      ...
      await writeAudit(db, { action: 'SELL_GIFT_CARD', actor: event.created_by, role: 'RECEPTION', centre_id: centreId, business_date: date, target_event_id: event.id, new_value: { code, value: giftCard.original_paise } })
    ...
    if (route === '/events/expense' && method === 'POST') {
      ...
      const event = {
        ...
        centre_id: centreId, business_date: date, created_at: new Date().toISOString(), created_by: b.created_by || 'manager',
      ...
      await writeAudit(db, { action: 'CREATE_EXPENSE', actor: event.created_by, role: 'MANAGER', centre_id: centreId, business_date: date, target_event_id: event.id, new_value: { category: event.category, amount: event.amount } })
      return cors(NextResponse.json(clean(event)))
    }
    ...
    if (route === '/events/cash-movement' && method === 'POST') {
      ...
      await writeAudit(db, { action: 'CASH_MOVEMENT', actor: event.created_by, role: 'MANAGER', centre_id: centreId, business_date: date, target_event_id: event.id, new_value: { movement_type: event.movement_type, amount: event.amount } })
      return cors(NextResponse.json(clean(event)))
    }
    ...
    if (route.startsWith('/events/') && route.endsWith('/reverse') && method === 'POST') {
      const id = route.split('/')[2]
      const { reason = '', actor = 'manager', role = 'MANAGER' } = await request.json()
      if (!['MANAGER', 'OPS', 'SUPER', 'SUPER_ADMIN', 'CENTRE_USER'].includes(role) && !isProtected) {
        return cors(NextResponse.json({ error: 'Manager+ required' }, { status: 403 }))
      }
      ...
      await writeAudit(db, { action: 'REVERSE_EVENT', actor, role, centre_id: original.centre_id, business_date: original.business_date, target_event_id: original.id, new_value: { reversed: true, reversal_event_id: reversal.id, reason } })
    ...
    if (route === '/business-day/close' && method === 'POST') {
      ...
      await writeAudit(db, { action: 'CLOSE_DAY', actor: b.actor || 'reception', role: b.role || 'RECEPTION', centre_id: centreId, business_date: date, previous_value: { status: 'OPEN' }, new_value: { status: 'CLOSED', declared, expected: agg.closing_cash_expected } })
    ...
    if (route === '/business-day/reopen' && method === 'POST') {
      const b = await request.json()
      if (isProtected && auth.profile.role !== 'SUPER_ADMIN' && b.role !== 'MANAGER' && b.role !== 'SUPER') {
        return cors(NextResponse.json({ error: 'Manager+ required' }, { status: 403 }))
      }
      const { data: bd } = await db.from('business_days').select('*').eq('centre_id', b.centre_id).eq('business_date', b.business_date).single()
    ...
    if (route === '/business-day/set-opening' && method === 'POST') {
      ...
      if (bd.status === 'CLOSED') return cors(NextResponse.json({ error: 'Day closed' }, { status: 400 }))
    ...
    if (route === '/dashboard' && method === 'GET') {
      ...
      for (const c of targetCentres) {
        ...
        const { data: evs } = await db.from('events').select('*').eq('centre_id', c.id).eq('business_date', date)
      ...
      const allEventsQuery = centre_id === 'ALL' 
        ? await db.from('events').select('*').eq('business_date', date)
        : await db.from('events').select('*').eq('centre_id', centre_id).eq('business_date', date)
      
      const consolidatedAgg = aggregate(cleanArr(allEventsQuery.data), totalOpening)
    ...
    if (route === '/master-register' && method === 'GET') {
      ...
      let evQuery = db.from('events').select('*').gte('business_date', from).lte('business_date', to)
      if (centre_id !== 'ALL') evQuery = evQuery.eq('centre_id', centre_id)
      const { data: evRaw } = await evQuery.order('created_at', { ascending: true })
      const events = cleanArr(evRaw)

      // Fetch all business_days records in range to get opening cash and status per date
      let bdQuery = db.from('business_days').select('*').gte('business_date', from).lte('business_date', to)
      if (centre_id !== 'ALL') bdQuery = bdQuery.eq('centre_id', centre_id)
      const { data: bdRaw } = await bdQuery
      const bds = cleanArr(bdRaw)

      // Build a map: business_date → { opening_cash, status }
      // When multiple centres contribute to a date (ALL mode), sum opening_cash values
      const bdByDate = {}
      for (const bd of bds) {
        const d = bd.business_date
        if (!bdByDate[d]) bdByDate[d] = { opening_cash: 0, status: bd.status }
        bdByDate[d].opening_cash += (bd.opening_cash || 0)
        // Status: OPEN if any centre is still open
        if (bd.status === 'OPEN') bdByDate[d].status = 'OPEN'
      }

      // Bucket events by business_date
      const buckets = {}
      ...
      // Collect all distinct dates (from both events and business_days)
      const allDates = new Set([...Object.keys(buckets), ...Object.keys(bdByDate)])
      const sortedDates = Array.from(allDates).sort()

      const rows = []
      for (const d of sortedDates) {
        const dayEvents = buckets[d] || []
        const bdInfo = bdByDate[d] || { opening_cash: 0, status: 'OPEN' }
        const agg = aggregate(dayEvents, bdInfo.opening_cash)
        rows.push({
          business_date: d,
          opening_cash: bdInfo.opening_cash,
          status: bdInfo.status,
          ...agg,
        })
      }

      return cors(NextResponse.json({ from, to, centre_id, count: rows.length, rows }))
    }
    ...
    if (route === '/reports/pl' && method === 'GET') {
      ...
      for (const p of Array.from(periods).sort()) {
        const perCentre = []
        let consolidatedEvents = [], consolidatedOpening = 0
        for (const c of targetCentres) {
          const key = p + '|' + c.id
          const evs = bucket.get(key) || [], opening = openings.get(key) || 0
          if (evs.length === 0 && opening === 0) continue
          perCentre.push({ centre_id: c.id, centre_name: c.name, opening_cash: opening, ...aggregate(evs, opening) })
          consolidatedEvents = consolidatedEvents.concat(evs)
          consolidatedOpening += opening
        }
        rows.push({ period: p, per_centre: perCentre, consolidated: { opening_cash: consolidatedOpening, ...aggregate(consolidatedEvents, consolidatedOpening) } })
      }
