  ...
  try {
    ...
    if (route === '/users' && method === 'POST') {
      ...
      const { data: created, error: createErr } = await db.auth.admin.createUser({
        email, password: password || 'DefaultPass123!', email_confirm: true, user_metadata: { full_name }
      })
    ...
    if (route === '/master-register' && method === 'GET') {
      ...
      const filter = { business_date: { $gte: from, $lte: to } }
      if (centre_id !== 'ALL') filter.centre_id = centre_id
      const events = await getEventsByFilter(db, filter)
      const { data: centres } = await db.from('centres').select('*').eq('active', true)
      const cMap = Object.fromEntries(centres.map(c => [c.id, clean(c)]))
      const enriched = events.map(ev => ({ ...ev, centre_name: cMap[ev.centre_id]?.name || 'Unknown', ledger_impact: {
        revenue: (ev.type==='BOOKING' && (ev.payment_method==='MEMBERSHIP'||ev.payment_method==='GIFT_CARD'))?0:(['BOOKING','MEMBERSHIP_SALE','GIFT_CARD_SALE'].includes(ev.type)?(ev.is_reversal?-1:1)*(ev.amount||0):0),
        expense: ev.type === 'EXPENSE' ? (ev.is_reversal?-1:1)*(ev.amount||0) : 0,
        cash: (ev.is_reversal?-1:1)*cashImpact(ev)
      }}))
      return cors(NextResponse.json({ from, to, centre_id, count: enriched.length, events: enriched }))
    }
