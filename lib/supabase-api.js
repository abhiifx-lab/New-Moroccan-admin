...
import { EVENT_TYPES, METRICS, aggregate, drillDown, businessDate, periodLabel, eventSign, cashImpact, paymentSplit, membershipRedemption, giftCardRedemption } from './financial-engine.js'
...
async function resolveCustomerId(db, { name, phone, email = '', centreId }) {
  const normalized = String(phone || '').replace(/\D/g, '')
  if (normalized.length < 10 || normalized.length > 15) return null
  const { data: existing } = await db.from('customers').select('*').eq('phone_normalized', normalized).single()
  if (existing) {
    await db.from('customers').update({ name, phone, email: email || existing.email, updated_at: new Date().toISOString() }).eq('id', existing.id)
    return existing.id
  }
  const id = uuidv4()
  const { error } = await db.from('customers').insert({ id, name, phone, phone_normalized: normalized, email: email || null, first_seen_centre_id: centreId, updated_at: new Date().toISOString() })
  if (error?.code === '23505') {
    const { data: raced } = await db.from('customers').select('id').eq('phone_normalized', normalized).single()
...
export async function handleSupabaseRoute(request, { params }) {
  ...
  try {
    ...
    if (route.startsWith('/users/') && method === 'DELETE') {
      ...
      if (!userId) return cors(NextResponse.json({ error: 'User ID is required' }, { status: 400 }))
      if (userId === auth.user.id) return cors(NextResponse.json({ error: 'You cannot delete your own signed-in account' }, { status: 400 }))

      const { data: existing } = await db.from('profiles').select('*').eq('id', userId).single()
      if (!existing) return cors(NextResponse.json({ error: 'User not found' }, { status: 404 }))
      const { error } = await db.auth.admin.deleteUser(userId)
      if (error) return cors(NextResponse.json({ error: `User deletion failed: ${error.message}` }, { status: 400 }))
      await writeAudit(db, { action: 'DELETE_USER', actor: auth.profile.email, role: 'SUPER_ADMIN', centre_id: existing.centre_id, business_date: businessDate(), new_value: { user_id: userId, email: existing.email, full_name: existing.full_name, role: existing.role } })
    ...
    if (route === '/events' && method === 'GET') {
      ...
      if ((q.type || '') === 'BOOKING' && events.length > 0) {
        const { data: bookingRows } = await db.from('bookings').select('*').in('event_id', events.map(e => e.id))
        const bookingByEvent = new Map(cleanArr(bookingRows).map(b => [b.event_id, b]))
        return cors(NextResponse.json(events.map(e => ({ ...e, booking: bookingByEvent.get(e.id) || null }))))
      }
      return cors(NextResponse.json(events))
    }

    // ---------------- THERAPIST DIRECTORY ----------------
    if (route === '/therapists' && method === 'GET') {
      const requestedCentre = auth.profile.role === 'CENTRE_USER' ? auth.profile.centre_id : (q.centre_id || 'ALL')
      const scope = enforceCentreReadScope(auth, requestedCentre)
      if (scope.error) return cors(NextResponse.json({ error: scope.error }, { status: scope.status }))
      let query = db.from('therapists').select('*').order('name')
      if (scope.centre_id !== 'ALL') query = query.eq('centre_id', scope.centre_id)
      const { data, error } = await query
      if (error) throw new Error(`Therapist directory error: ${error.message}`)
      return cors(NextResponse.json(cleanArr(data)))
    }

    if (route === '/therapists' && method === 'POST') {
      const b = await request.json()
      const scope = enforceCentreWriteScope(auth, b.centre_id)
      if (scope.error) return cors(NextResponse.json({ error: scope.error }, { status: scope.status }))
      if (!scope.centre_id || scope.centre_id === 'ALL') return cors(NextResponse.json({ error: 'Choose a specific centre' }, { status: 400 }))
      await validateCentre(db, scope.centre_id)
      const name = String(b.name || '').trim()
      if (name.length < 2) return cors(NextResponse.json({ error: 'Therapist name is required' }, { status: 400 }))
      const { data: duplicates } = await db.from('therapists').select('id').eq('centre_id', scope.centre_id).ilike('name', name).limit(1)
      if (duplicates?.length) return cors(NextResponse.json({ error: 'A therapist with this name already exists at this centre' }, { status: 400 }))
      const therapist = { id: uuidv4(), name, centre_id: scope.centre_id, active: true }
      const { data, error } = await db.from('therapists').insert(therapist).select().single()
      if (error) throw new Error(`Therapist creation failed: ${error.message}`)
      await writeAudit(db, { action: 'CREATE_THERAPIST', actor: auth.profile.email, role: operationalRole(auth), centre_id: scope.centre_id, business_date: businessDate(), new_value: { therapist_id: therapist.id, name, active: true } })
      return cors(NextResponse.json({ ok: true, therapist: clean(data) }))
    }

    if (route.startsWith('/therapists/') && method === 'PATCH') {
      const therapistId = route.split('/')[2]
      const b = await request.json()
      const { data: existing } = await db.from('therapists').select('*').eq('id', therapistId).single()
      if (!existing) return cors(NextResponse.json({ error: 'Therapist not found' }, { status: 404 }))
      const scope = enforceCentreWriteScope(auth, existing.centre_id)
      if (scope.error) return cors(NextResponse.json({ error: scope.error }, { status: scope.status }))
      const nextName = b.name == null ? existing.name : String(b.name).trim()
      if (nextName.length < 2) return cors(NextResponse.json({ error: 'Therapist name is required' }, { status: 400 }))
      const updates = { name: nextName, active: b.active == null ? existing.active : Boolean(b.active) }
      const { data, error } = await db.from('therapists').update(updates).eq('id', therapistId).select().single()
      if (error) throw new Error(`Therapist update failed: ${error.message}`)
      await writeAudit(db, { action: 'UPDATE_THERAPIST', actor: auth.profile.email, role: operationalRole(auth), centre_id: existing.centre_id, business_date: businessDate(), new_value: { therapist_id: therapistId, previous: { name: existing.name, active: existing.active }, ...updates } })
      return cors(NextResponse.json({ ok: true, therapist: clean(data) }))
    }

    if (route.startsWith('/therapists/') && method === 'DELETE') {
      const therapistId = route.split('/')[2]
      const { data: existing } = await db.from('therapists').select('*').eq('id', therapistId).single()
      if (!existing) return cors(NextResponse.json({ error: 'Therapist not found' }, { status: 404 }))
      const scope = enforceCentreWriteScope(auth, existing.centre_id)
      ...
      const { data: linkedBooking } = await db.from('bookings').select('id').eq('therapist_id', therapistId).limit(1).maybeSingle()
      if (linkedBooking) return cors(NextResponse.json({ error: 'This therapist has booking history. Deactivate them to preserve historical records.' }, { status: 409 }))
      const { error } = await db.from('therapists').delete().eq('id', therapistId)
      if (error) return cors(NextResponse.json({ error: `Therapist removal failed: ${error.message}` }, { status: 400 }))
      await writeAudit(db, { action: 'DELETE_THERAPIST', actor: auth.profile.email, role: operationalRole(auth), centre_id: existing.centre_id, business_date: businessDate(), new_value: { therapist_id: therapistId, name: existing.name } })
      return cors(NextResponse.json({ ok: true }))
    }

    // ---------------- APPOINTMENT & SALE WORKFLOW ----------------
    if (route === '/appointment/options' && method === 'GET') {
      const requestedCentre = auth.profile.role === 'CENTRE_USER' ? auth.profile.centre_id : q.centre_id
      const scope = enforceCentreReadScope(auth, requestedCentre)
      if (scope.error) return cors(NextResponse.json({ error: scope.error }, { status: scope.status }))
      if (!scope.centre_id || scope.centre_id === 'ALL') return cors(NextResponse.json({ error: 'Choose a specific centre' }, { status: 400 }))
      await validateCentre(db, scope.centre_id)

      const [{ data: variants, error: variantsError }, { data: therapists, error: therapistsError }] = await Promise.all([
        db.from('services').select('*').eq('active', true).or(`centre_id.is.null,centre_id.eq.${scope.centre_id}`).order('treatment_name').order('duration'),
        db.from('therapists').select('*').eq('centre_id', scope.centre_id).eq('active', true).order('name')
      ])
      if (variantsError) throw new Error(`Treatment options error: ${variantsError.message}`)
      if (therapistsError) throw new Error(`Therapist options error: ${therapistsError.message}`)
      const cleanVariants = cleanArr(variants)
      const centreVariants = cleanVariants.filter(variant => variant.centre_id === scope.centre_id)
      // A configured centre menu is authoritative. Shared defaults are only a
      // fallback for centres that do not yet have their own menu.
      return cors(NextResponse.json({ centre_id: scope.centre_id, variants: centreVariants.length ? centreVariants : cleanVariants.filter(variant => !variant.centre_id), therapists: cleanArr(therapists) }))
    }

    if (route === '/customers/lookup' && method === 'GET') {
      const phone = String(q.phone || '').replace(/\D/g, '')
      if (phone.length < 10 || phone.length > 15) return cors(NextResponse.json({ error: 'Enter a valid mobile number' }, { status: 400 }))
      const { data: customer } = await db.from('customers').select('*').eq('phone_normalized', phone).single()
      if (!customer) return cors(NextResponse.json({ found: false, customer: null, intelligence: null }))

      const { data: visitsRaw } = await db.from('events').select('*').eq('customer_id', customer.id).eq('event_type', 'BOOKING').eq('is_reversal', false).order('created_at', { ascending: false })
      const visits = cleanArr(visitsRaw)
      let { data: memberships } = await db.from('memberships').select('*').eq('customer_id', customer.id).eq('active', true).eq('reversed', false)
      let { data: giftCards } = await db.from('gift_cards').select('*').eq('customer_id', customer.id).eq('active', true).eq('reversed', false)
      if (!memberships?.length) ({ data: memberships } = await db.from('memberships').select('*').eq('buyer', customer.name).eq('active', true).eq('reversed', false))
      if (!giftCards?.length) ({ data: giftCards } = await db.from('gift_cards').select('*').eq('buyer', customer.name).eq('active', true).eq('reversed', false))
      const therapistCounts = visits.reduce((acc, visit) => { if (visit.therapist) acc[visit.therapist] = (acc[visit.therapist] || 0) + 1; return acc }, {})
      const preferredTherapist = Object.entries(therapistCounts).sort((a,b) => b[1] - a[1])[0]?.[0] || null
      return cors(NextResponse.json({
        found: true,
        customer: clean(customer),
        memberships: cleanArr(memberships),
        gift_cards: cleanArr(giftCards),
        intelligence: {
          previous_visits: visits.length,
          last_visit: visits[0]?.booking_time || visits[0]?.created_at || null,
          preferred_therapist: preferredTherapist,
          lifetime_spend_paise: visits.reduce((sum, visit) => sum + Number(visit.amount || 0), 0),
          membership_balance_paise: cleanArr(memberships).reduce((sum, item) => sum + Number(item.remaining_paise || 0), 0),
          gift_card_balance_paise: cleanArr(giftCards).reduce((sum, item) => sum + Number(item.remaining_paise || 0), 0),
        }
      }))
    }

    if (route === '/offers/validate' && method === 'POST') {
      const b = await request.json()
      const requestedCentre = auth.profile.role === 'CENTRE_USER' ? auth.profile.centre_id : b.centre_id
      const scope = enforceCentreReadScope(auth, requestedCentre)
      if (scope.error) return cors(NextResponse.json({ error: scope.error }, { status: scope.status }))
      const code = String(b.code || '').trim()
      const { data: variant } = await db.from('services').select('*').eq('id', b.service_id).eq('active', true).single()
      if (!variant || (variant.centre_id && variant.centre_id !== scope.centre_id)) return cors(NextResponse.json({ error: 'Select a valid treatment variant first' }, { status: 400 }))
      const today = businessDate()
      const { data: offers } = await db.from('offers').select('*').ilike('code', code).eq('active', true)
      const offer = cleanArr(offers).find(o => (!o.centre_id || o.centre_id === scope.centre_id) && (!o.valid_from || o.valid_from <= today) && (!o.valid_to || o.valid_to >= today))
      if (!offer) return cors(NextResponse.json({ error: 'Coupon or offer code is invalid or expired' }, { status: 400 }))
      if ((variant.price_paise || 0) < (offer.min_spend_paise || 0)) return cors(NextResponse.json({ error: 'Minimum spend requirement is not met for this offer' }, { status: 400 }))
      let discount = offer.discount_type === 'PERCENT' ? Math.floor(variant.price_paise * offer.discount_value / 100) : offer.discount_value
      if (offer.max_discount_paise != null) discount = Math.min(discount, offer.max_discount_paise)
      discount = Math.min(discount, variant.price_paise)
      return cors(NextResponse.json({ ok: true, offer: clean(offer), discount_paise: discount, amount_after_discount_paise: variant.price_paise - discount }))
    }

    if (route === '/appointments' && method === 'POST') {
      const b = await request.json()
      const requestedCentre = auth.profile.role === 'CENTRE_USER' ? auth.profile.centre_id : b.centre_id
      const scope = enforceCentreWriteScope(auth, requestedCentre)
      if (scope.error) return cors(NextResponse.json({ error: scope.error }, { status: scope.status }))
      if (!scope.centre_id || scope.centre_id === 'ALL') return cors(NextResponse.json({ error: 'Choose a specific centre' }, { status: 400 }))
      await validateCentre(db, scope.centre_id)
      const saleDate = businessDate()
      const day = await ensureBusinessDay(db, scope.centre_id, saleDate)
      if (day.status === 'CLOSED') return cors(NextResponse.json({ error: 'Business day is closed. Authorized reopen required.' }, { status: 400 }))
      const { data, error } = await db.rpc('create_appointment_sale', {
        p_centre_id: scope.centre_id,
        p_business_date: saleDate,
        p_customer_phone: b.customer_phone,
        p_customer_name: b.customer_name,
        p_customer_email: b.customer_email || '',
        p_service_id: b.service_id,
        p_therapist_id: b.therapist_id,
        p_appointment_date: b.appointment_date,
        p_appointment_time: b.appointment_time,
        p_offer_code: b.offer_code || '',
        p_membership_code: b.membership_code || '',
        p_membership_redemption_paise: Number(b.membership_redemption_paise || 0),
        p_gift_card_code: b.gift_card_code || '',
        p_gift_card_redemption_paise: Number(b.gift_card_redemption_paise || 0),
        p_payment_method: b.payment_method,
        p_physical_receipt_no: b.physical_receipt_no || '',
        p_created_by: auth.profile.email,
    ...
    if (route === '/bookings/export' && method === 'GET') {
      const scope = enforceCentreReadScope(auth, q.centre_id || 'ALL')
      if (scope.error) return cors(NextResponse.json({ error: scope.error }, { status: scope.status }))
      let query = db.from('bookings').select('*').gte('appointment_date', q.from || q.date || businessDate()).lte('appointment_date', q.to || q.date || businessDate()).order('appointment_date').order('appointment_time')
      if (scope.centre_id !== 'ALL') query = query.eq('centre_id', scope.centre_id)
      const { data: rows, error } = await query
      if (error) throw new Error(`Booking export failed: ${error.message}`)
      const { data: centres } = await db.from('centres').select('id,name')
      const centreNames = new Map(cleanArr(centres).map(c => [c.id, c.name]))
      const csv = toCsv(cleanArr(rows), [
        { label: 'Appointment Date', get: r => r.appointment_date }, { label: 'Appointment Time', get: r => r.appointment_time },
        { label: 'Centre', get: r => centreNames.get(r.centre_id) || r.centre_id }, { label: 'Physical Receipt No.', get: r => r.physical_receipt_no },
        { label: 'Mobile', get: r => r.customer_phone }, { label: 'Customer', get: r => r.customer }, { label: 'Email', get: r => r.customer_email },
        { label: 'Treatment', get: r => r.service_name }, { label: 'Therapist', get: r => r.therapist }, { label: 'Duration', get: r => r.duration_minutes },
        { label: 'Base Price (₹)', get: r => (r.base_price_paise/100).toFixed(2) }, { label: 'Offer', get: r => r.offer_code },
        { label: 'Discount (₹)', get: r => (r.discount_paise/100).toFixed(2) }, { label: 'Membership Redemption (₹)', get: r => (r.membership_redemption_paise/100).toFixed(2) },
        { label: 'Gift Card Redemption (₹)', get: r => (r.gift_card_redemption_paise/100).toFixed(2) }, { label: 'Final Receivable (₹)', get: r => (r.final_receivable_paise/100).toFixed(2) },
        { label: 'Status', get: r => r.status }
      ])
      return cors(new NextResponse(csv, { status: 200, headers: { 'Content-Type': 'text/csv', 'Content-Disposition': 'attachment; filename="appointments.csv"' } }))
    }

    if (route.startsWith('/bookings/') && route.endsWith('/receipt') && method === 'PATCH') {
      ...
      const b = await request.json()
      const receipt = String(b.physical_receipt_no || '').trim()
      const { data: booking } = await db.from('bookings').select('*').eq('event_id', eventId).single()
      if (!booking) return cors(NextResponse.json({ error: 'Booking not found' }, { status: 404 }))
      if (auth.profile.role === 'CENTRE_USER' && booking.centre_id !== auth.profile.centre_id) return cors(NextResponse.json({ error: 'Forbidden' }, { status: 403 }))
      const { data: updated, error } = await db.from('bookings').update({ physical_receipt_no: receipt || null, updated_at: new Date().toISOString() }).eq('id', booking.id).select().single()
      if (error?.code === '23505') return cors(NextResponse.json({ error: 'Physical receipt number already exists at this centre' }, { status: 400 }))
      if (error) throw new Error(`Receipt update failed: ${error.message}`)
      await writeAudit(db, { action: 'UPDATE_PHYSICAL_RECEIPT', actor: auth.profile.email, role: manager.role, centre_id: booking.centre_id, business_date: booking.business_date, target_event_id: eventId, new_value: { previous: booking.physical_receipt_no, physical_receipt_no: receipt || null } })
      return cors(NextResponse.json({ ok: true, booking: clean(updated) }))
    }
    ...
    if (route === '/events/membership' && method === 'POST') {
      ...
      const pricePaise = Number(b.price_paise)
      const valuePaise = Number(b.value_paise)
      if (!buyer) return cors(NextResponse.json({ error: 'Customer is required' }, { status: 400 }))
      if (!Number.isSafeInteger(pricePaise) || pricePaise <= 0) return cors(NextResponse.json({ error: 'Sale price must be greater than zero' }, { status: 400 }))
      if (!Number.isSafeInteger(valuePaise) || valuePaise <= 0) return cors(NextResponse.json({ error: 'Membership value must be greater than zero' }, { status: 400 }))
      let centreId = b.centre_id
      if (isProtected) {
        const scope = enforceCentreWriteScope(auth, b.centre_id)
        if (scope.error) return cors(NextResponse.json({ error: scope.error }, { status: scope.status }))
      ...
      const event = {
        ...
        customer: buyer, amount: pricePaise, payment_method: b.payment_method, payment_breakdown: b.payment_breakdown || null,
        membership_code: code, is_reversal: false, reverses: null, notes: b.notes || ''
      }
      const membership = {
        code, buyer, recipient: String(b.recipient || buyer).trim(),
        original_paise: valuePaise, remaining_paise: valuePaise,
        price_paise: pricePaise, sold_at_centre_id: centreId, sold_at_date: date,
        payment_method: b.payment_method, active: true, reversed: false, source_event_id: event.id, redemptions: []
      }
      membership.customer_id = await resolveCustomerId(db, { name: buyer, phone: b.customer_phone, centreId })
    ...
    if (route === '/events/gift-card' && method === 'POST') {
      const b = await request.json()
      const buyer = String(b.buyer || '').trim()
      const recipient = String(b.recipient || buyer).trim()
      const pricePaise = Number(b.price_paise)
      const valuePaise = Number(b.value_paise)
      if (!buyer) return cors(NextResponse.json({ error: 'Buyer is required' }, { status: 400 }))
      if (!recipient) return cors(NextResponse.json({ error: 'Recipient is required' }, { status: 400 }))
      if (!Number.isSafeInteger(pricePaise) || pricePaise <= 0) return cors(NextResponse.json({ error: 'Sale price must be greater than zero' }, { status: 400 }))
      if (!Number.isSafeInteger(valuePaise) || valuePaise <= 0) return cors(NextResponse.json({ error: 'Gift card value must be greater than zero' }, { status: 400 }))
      let centreId = b.centre_id
      if (isProtected) {
        const scope = enforceCentreWriteScope(auth, b.centre_id)
        if (scope.error) return cors(NextResponse.json({ error: scope.error }, { status: scope.status }))
      ...
      const giftCard = {
        code, buyer, recipient,
        original_paise: valuePaise, remaining_paise: valuePaise,
        price_paise: pricePaise, sold_at_centre_id: centreId, sold_at_date: date,
        payment_method: b.payment_method, active: true, reversed: false, source_event_id: event.id, redemptions: []
      }
      giftCard.customer_id = await resolveCustomerId(db, { name: recipient, phone: b.customer_phone, centreId })
      await db.from('events').insert(event)
    ...
    if (route.startsWith('/events/') && route.endsWith('/reverse') && method === 'POST') {
      ...
      if (original.type === 'BOOKING') {
        const membershipAmount = membershipRedemption(original)
        const giftCardAmount = giftCardRedemption(original)
        const membershipCode = original.metadata?.membership_code
        const giftCardCode = original.metadata?.gift_card_code
        if (membershipAmount > 0 && membershipCode) {
          const { data: m } = await db.from('memberships').select('*').eq('code', membershipCode).single()
          if (m) await db.from('memberships').update({ remaining_paise: m.remaining_paise + membershipAmount, redemption_count: Math.max(0, (m.redemption_count || 1) - 1) }).eq('code', membershipCode)
        }
        if (giftCardAmount > 0 && giftCardCode) {
          const { data: g } = await db.from('gift_cards').select('*').eq('code', giftCardCode).single()
          if (g) await db.from('gift_cards').update({ remaining_paise: g.remaining_paise + giftCardAmount, redemption_count: Math.max(0, (g.redemption_count || 1) - 1) }).eq('code', giftCardCode)
        }
        await db.from('bookings').update({ status: 'REVERSED', updated_at: new Date().toISOString() }).eq('event_id', original.id)
      }
    ...
    if (route === '/drill-down' && method === 'GET') {
      ...
      if (metric === 'closing_cash_expected') {
        const openingDate = q.from || date
        let openingQuery = db.from('business_days').select('centre_id, opening_cash').eq('business_date', openingDate)
        if (centre_id !== 'ALL') openingQuery = openingQuery.eq('centre_id', centre_id)
        const { data: openingRows } = await openingQuery
        const cleanOpeningRows = cleanArr(openingRows)
        const openingCash = cleanOpeningRows.reduce((sum, row) => sum + Number(row.opening_cash || 0), 0)
        res.total += openingCash
        res.opening_cash = openingCash
        if (openingCash || cleanOpeningRows.length) {
          res.breakdown = {
            OPENING_BALANCE: { count: cleanOpeningRows.length, total: openingCash },
            ...res.breakdown,
          }
        }
      }
    ...
    if (route === '/master-register' && method === 'GET') {
      ...
      for (const c of targetCentres) {
        ...
        for (const d of sortedDatesForCentre) {
          ...
          const declaredClosing = bd?.closing_cash_declared ?? bd?.actual_closing_cash ?? null
          // The register's closing balance is the physical cash carried into the
          // next business day after close. While a day is open, show the live
          // expected balance because no physical closing has been declared yet.
          const closingCashBalance = bd?.status === 'CLOSED' && declaredClosing != null
            ? declaredClosing
            : agg.closing_cash_expected
          const rowObj = {
            business_date: d,
            centre_id: c.id,
            ...
            ...agg,
            closing_cash_declared: declaredClosing,
            closing_cash_balance: closingCashBalance,
            cash_variance: declaredClosing == null ? null : declaredClosing - agg.closing_cash_expected
          }
          cRows.push(rowObj)

          // Carry forward closing cash to next day's opening cash
          runningOpeningCash = closingCashBalance
        }
      ...
      if (targetCentres.length === 1) {
        ...
        for (const d of allSortedDates) {
          ...
          const combinedRow = {
            ...
            closing_cash_declared: centreDayRows.every(r => r.closing_cash_declared != null)
              ? centreDayRows.reduce((s, r) => s + r.closing_cash_declared, 0)
              : null,
            closing_cash_balance: centreDayRows.reduce((s, r) => s + (r.closing_cash_balance || 0), 0),
            cash_variance: centreDayRows.every(r => r.cash_variance != null)
              ? centreDayRows.reduce((s, r) => s + r.cash_variance, 0)
              : null,
