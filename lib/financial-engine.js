...
export const METRICS = {
  ...
  online_sales: {
    ...
    filter: e => ['BOOKING','MEMBERSHIP_SALE','GIFT_CARD_SALE'].includes(e.type)
      && (upi1Portion(e) + upi2Portion(e) + cardPortion(e)) > 0,
    contribute: e => upi1Portion(e) + upi2Portion(e) + cardPortion(e),
  },
  membership_redemption_value: {
    label: 'Membership Redemption (operational)',
    filter: e => e.type === 'BOOKING' && membershipRedemption(e) > 0,
    contribute: e => membershipRedemption(e),
  },
  gift_card_redemption_value: {
    label: 'Gift Card Redemption (operational)',
    filter: e => e.type === 'BOOKING' && giftCardRedemption(e) > 0,
    contribute: e => giftCardRedemption(e),
  },
  total_expenses: {
    label: 'Expenses',
  ...
  other_cash_in:     { label: 'Cash Received',    filter: e => e.type === 'CASH_MOVEMENT' && e.movement_type === 'CASH_RECEIVED',     contribute: e => e.amount || 0 },
  other_cash_out:    { label: 'Cash Handed Over', filter: e => e.type === 'CASH_MOVEMENT' && e.movement_type === 'CASH_HANDED_OVER',  contribute: e => e.amount || 0 },
  bookings:          { label: 'Bookings',      filter: e => e.type === 'BOOKING', contribute: () => 1, isCount: true },
  redemptions:       { label: 'Redemptions',   filter: e => e.type === 'BOOKING' && (membershipRedemption(e) > 0 || giftCardRedemption(e) > 0), contribute: () => 1, isCount: true },
...
function membershipRedemption(e) {
  const stored = Number(e?.metadata?.membership_redemption_paise || 0)
  return stored > 0 ? stored : (e?.payment_method === 'MEMBERSHIP' ? Number(e.amount || 0) : 0)
}
function giftCardRedemption(e) {
  const stored = Number(e?.metadata?.gift_card_redemption_paise || 0)
  return stored > 0 ? stored : (e?.payment_method === 'GIFT_CARD' ? Number(e.amount || 0) : 0)
}
...
export function aggregate(events, openingCashPaise = 0) {
  ...
  for (const ev of events) {
    ...
    if (ev.type === 'BOOKING') {
      ...
      if (membershipRedeemed > 0 || giftCardRedeemed > 0) R.redemptions += s
      R.membership_redemption_value += s * membershipRedeemed
      R.gift_card_redemption_value += s * giftCardRedeemed
      if (ev.payment_method === 'MEMBERSHIP' && (ev.amount || 0) === 0) {
        // Fully covered by stored-value redemption; revenue was recognised at sale.
      } else if (ev.payment_method === 'GIFT_CARD' && (ev.amount || 0) === 0) {
        // Fully covered by stored-value redemption; revenue was recognised at sale.
      } else {
...
export function drillDown(events, metric) {
  ...
  for (const item of items) {
    const t = item.event.type || 'OTHER'
    if (!breakdown[t]) breakdown[t] = { count: 0, total: 0 }
    breakdown[t].count += 1
    breakdown[t].total += item.contribution
  }
  return { events: items, total, breakdown, isCount: !!m.isCount, unique: m.unique || null, label: m.label }
}
...
export { cashImpact, paymentSplit, membershipRedemption, giftCardRedemption }
