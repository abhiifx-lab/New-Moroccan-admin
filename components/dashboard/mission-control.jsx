...
function SectionHeading({ title, subtitle, control }) {
  return <CardHeader className="flex-row items-start justify-between gap-4 p-5 pb-2"><div><CardTitle className="text-base font-bold text-slate-950">{title}</CardTitle>{subtitle && <p className="mt-1 text-xs text-slate-500">{subtitle}</p>}</div>{control}</CardHeader>
}
...
export function DashboardMissionControl({ centre, profile, refreshTick, onDrill, onNavigateAction, apiGet, formatMoney, today }) {
  ...
  const timeline = useMemo(()=>bookings.filter(event=>!event.is_reversal).sort((a,b)=>String(a.booking?.appointment_time||a.created_at).localeCompare(String(b.booking?.appointment_time||b.created_at))).slice(0,5),[bookings])
  const topServices = useMemo(()=>{
    const totals = new Map()
    for (const event of serviceEvents) {
      if (event.is_reversal) continue
  ...
  return (
    <div className="mission-dashboard space-y-5 text-slate-800">
      ...
      <div className="grid gap-5 xl:grid-cols-[1.45fr_1fr]">
        <Card className="border-white/80 bg-white/90 shadow-[0_12px_35px_rgba(49,46,129,0.06)]">
      ...
      <Card className="overflow-hidden border-white/80 bg-white/90 shadow-[0_12px_35px_rgba(49,46,129,0.06)]">
